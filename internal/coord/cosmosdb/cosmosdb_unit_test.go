...
func TestLockKeyDerivation(t *testing.T) {
	...
	// The key is used as both the Cosmos document id and partition key, which
	// forbid '/', '\\', '?' and '#'. Hashing must keep those out of the key.
	if suffix := strings.TrimPrefix(k1, "rss2msg:coord:"); strings.ContainsAny(suffix, `/\?#`) {
		t.Fatalf("lock key contains characters Cosmos forbids in id: %q", k1)
	}
}
