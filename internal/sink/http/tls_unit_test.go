...
func TestBuildTLSConfig(t *testing.T) {
	...
	t.Run("empty options leave ServerName empty (net/http fills from URL)", func(t *testing.T) {
	...
	t.Run("server name override", func(t *testing.T) {
		tc, err := buildTLSConfig(TLSOptions{ServerName: "hook.example.com"})
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if tc.ServerName != "hook.example.com" {
			t.Errorf("ServerName = %q, want hook.example.com", tc.ServerName)
		}
	})
	...
	t.Run("New applies TLS config to the client transport", func(t *testing.T) {
		p, err := New(Options{
			Name: "hook",
		...
		tr, ok := p.client.Transport.(*stdhttp.Transport)
		if !ok {
			t.Fatalf("client.Transport = %T, want *http.Transport", p.client.Transport)
		}
		if tr.TLSClientConfig == nil || !tr.TLSClientConfig.InsecureSkipVerify {
			t.Error("transport TLSClientConfig not applied")
		}
	})
}
