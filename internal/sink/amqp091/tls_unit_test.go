...
func TestBuildTLSConfig(t *testing.T) {
	...
	t.Run("empty options leave ServerName empty (amqp091 fills from URI host)", func(t *testing.T) {
	...
	t.Run("server name override", func(t *testing.T) {
		tc, err := buildTLSConfig(TLSOptions{ServerName: "broker.example.com"})
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if tc.ServerName != "broker.example.com" {
			t.Errorf("ServerName = %q, want broker.example.com", tc.ServerName)
		}
	})
