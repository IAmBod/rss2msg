...
summary: Persist seen-item state and HTTP cache validators with the sqlite, postgres, or dynamodb state store.
updated: 2026-06-09
---
...
> distributed coordinator (`redis`, `postgres`, or `dynamodb`), use `postgres` or
> `dynamodb` for state too. Validation warns if it sees a distributed coordinator
