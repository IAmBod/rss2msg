...
`memory` is the default and needs no shared backend. The `postgres`, `redis`, and
`dynamodb` coordinators serialise polling across instances and have **no leader
election** — losing instances skip the cycle silently. The `postgres` coordinator
reuses the state DSN by default; the `redis` coordinator offers `single` (default),
`sentinel` (tested), and `cluster` (best-effort) topologies.
