...
TLS applies to the Postgres state store, the Postgres coordinator, the Redis coordinator, and the network message sinks (Postgres, Kafka, amqp091, HTTP, gRPC); the field surface is identical across them.
...
The **postgres**, **kafka**, **amqp091**, **nats**, **http**, and **grpc** sinks accept the
...
- **amqp091** — dials with TLS; use an `amqps://` URL.
